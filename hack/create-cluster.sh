...
[ -x docker ] && echo "Please install docker CLI before continuing, see README.md for instructions" >&2 && exit 1
...
if ! kind get clusters | grep "${cluster_name}" > /dev/null; then
  kind create cluster --name "${cluster_name}" --kubeconfig="${kubeconfig_file}" --config=./hack/kind-config.yaml
else
